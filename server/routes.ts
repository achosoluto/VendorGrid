...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Notification endpoints for login-based alerts
  app.get('/api/notifications/login', readLimiter, isAuthenticated, async (req: any, res) => {
    try {
      const sessionId = req.headers['x-session-id'] || req.user.claims.sub + '_' + Date.now();
      const notifications = governmentDataMonitor.getLoginNotifications(String(sessionId));
      res.json({ notifications, sessionId });
    } catch (error) {
  ...
  app.post('/api/notifications/:id/acknowledge', writeLimiter, isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
  ...
  app.get('/api/system/status', readLimiter, async (req, res) => {
    try {
      const status = governmentDataMonitor.getSystemStatus();
      res.json(status);
    } catch (error) {
  ...
  // Register Government Data Integration Agent routes
  app.use(governmentDataRoutes);

  // Register Vendor Claiming routes
  app.use('/api/vendor-claiming', vendorClaimingRoutes);
